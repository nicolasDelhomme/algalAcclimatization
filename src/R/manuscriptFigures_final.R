...
#' author: "Nicolas Delhomme and Fai Theerarat Kochakarn"
#' date: "`r Sys.Date()`"
#' output:
#'  html_document:
#'    toc: true
...
pthw <- read_tsv(here("data/analysis/annotation/kegg-pathways-dictionary.tsv"))
...
eCannot <- read_tsv(here("data/analysis/annotation/kegg-transcript-pathway-mapping.tsv"))
...
kAnnot <- read_tsv(here("data/analysis/annotation/algae_KEGG_annotation.tsv"))
...
# get table of goicold per gene
temp <- names(degList)
degList <- lapply(X = names(degList), FUN = function(x) {
	degList[[x]] %>% mutate(Time = x)
} )
names(degList) <- temp
temp <- rbindlist(degList)
degListCold <- temp[temp$ID %in% goicold,]
degListCold$up <- degListCold$log2FoldChange > 0

temp <- names(degListPerTimePoint)
degListPerTimePoint <- lapply(X = names(degListPerTimePoint), FUN = function(x) {
	degListPerTimePoint[[x]] %>% mutate(Time = x)
} )
names(degListPerTimePoint) <- temp
temp <- rbindlist(degListPerTimePoint)
degListPerTimePointCold <- temp[temp$ID %in% goicold,]
degListPerTimePointCold$up <- degListPerTimePointCold$log2FoldChange > 0

write.table(degListCold,here("doc/degListCold.tsv"),quote = FALSE,sep = "\t",row.names = FALSE)
write.table(degListPerTimePointCold,here("doc/degListPerTimePointCold.tsv"),quote = FALSE,sep = "\t",row.names = FALSE)
...
ggdraw() +
	draw_image(
		here("ko00061.fatty-acid-biosynthesis.multi.png"))
...
pathview(
  ...
  species = pspecies,
  pathway.id = pnumber,
  out.suffix = "Carbon_fixation_in_photosynthetic_organisms",
  kegg.dir=here("data/analysis/kegg"),
  low=c("blue","blue"))

# ggdraw() +
# 	draw_image(
# 		here("ko00710.Carbon_fixation_in_photosynthetic_organisms.multi.png"))
...
pathview(
	...
	species = pspecies,
	pathway.id = pnumber,
	out.suffix = "Glycolysis_gluconeogenesis",
	kegg.dir=here("data/analysis/kegg"),
	low=c("blue","blue"))

# ggdraw() +
# 	draw_image(
# 		here("ko00010.Glycolysis_gluconeogenesis.multi.png"))
...
pathview(
	...
	species = pspecies,
	pathway.id = pnumber,
	out.suffix = "Pentose_phosphate_pathway",
	kegg.dir=here("data/analysis/kegg"),
	low=c("blue","blue"))

# ggdraw() +
# 	draw_image(
# 		here("ko00030.Pentose_phosphate_pathway.multi.png"))
...
pathview(
	...
	species = pspecies,
	pathway.id = pnumber,
	out.suffix = "Glyoxylate_and_dicarboxylate_metabolism",
	kegg.dir=here("data/analysis/kegg"),
	low=c("blue","blue"))

# ggdraw() +
# 	draw_image(
# 		here("ko00630.Glyoxylate_and_dicarboxylate_metabolism.multi.png"))

#Test getting them all together

eCannot <- read_tsv(here("data/analysis/annotation/kegg-transcript-pathway-mapping.tsv"))
kAnnot <- read_tsv(here("data/analysis/annotation/algae_KEGG_annotation.tsv"))

#Calvin cycle #Glycolysis #TCA #pentose phosphate #glyoxylate
pwy <- keggGet(c("ko00710","ko00010","ko00020","ko00030","ko00630"))
...
ko <- pwy[[1]]$ORTHOLOGY
ko <- ko[grepl("EC",ko)]
tb1 <- tibble(KO=names(ko),EC=ko) %>%
	mutate(EC=sub("]","",sub(".*EC:","",EC))) %>% 
	separate_longer_delim(EC," ")

ko <- pwy[[2]]$ORTHOLOGY
ko <- ko[grepl("EC",ko)]
tb2 <- tibble(KO=names(ko),EC=ko) %>%
	mutate(EC=sub("]","",sub(".*EC:","",EC))) %>% 
	separate_longer_delim(EC," ")

ko <- pwy[[3]]$ORTHOLOGY
ko <- ko[grepl("EC",ko)]
tb3 <- tibble(KO=names(ko),EC=ko) %>%
	mutate(EC=sub("]","",sub(".*EC:","",EC))) %>% 
	separate_longer_delim(EC," ")

ko <- pwy[[4]]$ORTHOLOGY
ko <- ko[grepl("EC",ko)]
tb4 <- tibble(KO=names(ko),EC=ko) %>%
	mutate(EC=sub("]","",sub(".*EC:","",EC))) %>% 
	separate_longer_delim(EC," ")

ko <- pwy[[5]]$ORTHOLOGY
ko <- ko[grepl("EC",ko)]
tb5 <- tibble(KO=names(ko),EC=ko) %>%
	mutate(EC=sub("]","",sub(".*EC:","",EC))) %>% 
	separate_longer_delim(EC," ")
...
#' From the annotation
ez <- eCannot %>% mutate(EC=str_sub(EC,4)) %>% 
	filter(EC %in% (tb %>% select(EC) %>% distinct() %>% unlist(use.names=FALSE)) & PID %in% c("ec00710","ec00010","ec00020","ec00030","ec00630"))
...
#read in the log2fc
acute <- read.table("/mnt/picea/projects/algae/cfunk/algal-acclimatization/analysis/DE/Response/acute_vs_control_results.csv",header = TRUE,sep = ",")
early <- read.table("/mnt/picea/projects/algae/cfunk/algal-acclimatization/analysis/DE/Response/early_vs_control_results.csv",header = TRUE,sep = ",")
latee <- read.table("/mnt/picea/projects/algae/cfunk/algal-acclimatization/analysis/DE/Response/late_vs_control_results.csv",header = TRUE,sep = ",")

sum(acute$X == early$X)
sum(acute$X == latee$X)
log2fc <- cbind(acute$X,acute$log2FoldChange,early$log2FoldChange,latee$log2FoldChange)
colnames(log2fc) <- c("TxID","acute","early","late")
...
ez %<>% left_join(log2fc,by="TxID")

ez$acute<- as.numeric(ez$acute)
ez$early<- as.numeric(ez$early)
ez$late<- as.numeric(ez$late)
...
pathview(
	...
	species = pspecies,
	pathway.id = pnumber,
	out.suffix = "Carbon_fixation_in_photosynthetic_organisms_log2FC",
	kegg.dir=here("data/analysis/kegg"),
	low=c("blue","blue"))
...
pathview(
	...
	species = pspecies,
	pathway.id = pnumber,
	out.suffix = "Carbon_fixation_in_photosynthetic_organisms_log2FC_acute",
	kegg.dir=here("data/analysis/kegg"),
	low=c("blue","blue"))
...
pathview(
	...
	species = pspecies,
	pathway.id = pnumber,
	out.suffix = "Carbon_fixation_in_photosynthetic_organisms_log2FC_early",
	kegg.dir=here("data/analysis/kegg"),
	low=c("blue","blue"))
...
pathview(
	...
	species = pspecies,
	pathway.id = pnumber,
	out.suffix = "Carbon_fixation_in_photosynthetic_organisms_log2FC_late",
	kegg.dir=here("data/analysis/kegg"),
	low=c("blue","blue"))
...
# get table of goicellwall per gene
temp <- names(degList)
degList <- lapply(X = names(degList), FUN = function(x) {
	degList[[x]] %>% mutate(Time = x)
} )
names(degList) <- temp
temp <- rbindlist(degList)
degListCellwall <- temp[temp$ID %in% goicellwall,]
degListCellwall$up <- degListCellwall$log2FoldChange > 0
...
names(degListPerTimePoint) <- temp
temp <- rbindlist(degListPerTimePoint)
degListPerTimePointCellwall <- temp[temp$ID %in% goicellwall,]
degListPerTimePointCellwall$up <- degListPerTimePointCellwall$log2FoldChange > 0

write.table(degListCellwall,here("doc/degListCellwall.tsv"),quote = FALSE,sep = "\t",row.names = FALSE)
write.table(degListPerTimePointCellwall,here("doc/degListPerTimePointCellwall.tsv"),quote = FALSE,sep = "\t",row.names = FALSE)
...
# get table of Olivia selected goi per gene
goi <- c("TRINITY_DN9587_c0_g1_i1",
				 "TRINITY_DN10823_c0_g1_i1",
				 "TRINITY_DN11365_c0_g1_i5",
				 "TRINITY_DN12229_c0_g1_i1",
				 "TRINITY_DN1815_c1_g1_i1",
				 "TRINITY_DN1815_c1_g1_i3",
				 "TRINITY_DN1815_c1_g1_i4",
				 "TRINITY_DN5479_c0_g1_i1",
				 "TRINITY_DN5479_c0_g1_i2",
				 "TRINITY_DN598_c0_g2_i1",
				 "TRINITY_DN635_c0_g1_i3",
				 "TRINITY_DN9587_c0_g1_i1")
temp <- names(degList)
degList <- lapply(X = names(degList), FUN = function(x) {
	degList[[x]] %>% mutate(Time = x)
} )
names(degList) <- temp
temp <- rbindlist(degList)
degListGoi <- temp[temp$ID %in% goi,]
degListGoi$up <- degListGoi$log2FoldChange > 0
...
names(degListPerTimePoint) <- temp
temp <- rbindlist(degListPerTimePoint)
degListPerTimePointGoi <- temp[temp$ID %in% goi,]
degListPerTimePointGoi$up <- degListPerTimePointGoi$log2FoldChange > 0

write.table(degListGoi,here("doc/degListGoi.tsv"),quote = FALSE,sep = "\t",row.names = FALSE)
write.table(degListPerTimePointGoi,here("doc/degListPerTimePointGoi.tsv"),quote = FALSE,sep = "\t",row.names = FALSE)
